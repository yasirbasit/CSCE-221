...
// Default constructor
StressBall::Stress_ball() : color(Stress_ball_colors(rand() % 4)), size(Stress_ball_sizes(rand() % 3)) {}
StressBall::Stressball(Stress_ball_colors c, Stress_ball_sizes s) : color(c), size(s) {}

// Destructor
StressBall::~Stressball()

// Get color of the stress ball
Stress_ball_colors StressBall::get_color() const {
	return this -> color;
}

// Get size of the stress ball
Stress_ball_sizes StressBall::get_size() const {
	return this -> size;
}
...
bool StressBall::operator==(const StressBall& sb) {
		return this -> get_color() == sb.get_color() && this -> get_size() == sb.get_size();
}


ostream& operator<<(std::ostream& o, const StressBall& sb) {
	o << "(";
	switch (sb.get_color()) {
